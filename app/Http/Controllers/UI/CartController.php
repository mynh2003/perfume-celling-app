...
class CartController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('ui.carts.index', compact('cartItems', 'totalQuantity', 'categories', 'brands'));
    }

    public function updateQuantity(Request $request, $productId)
    {
        // Kiểm tra xem sản phẩm có trong giỏ hàng của người dùng hiện tại không
        $cartItem = Cart::where('user_id', Auth::id())->where('product_id', $productId)->first();

        if (!$cartItem) {
            // Trả về lỗi nếu sản phẩm không có trong giỏ hàng của người dùng
        ...
        // Kiểm tra số lượng có hợp lệ không (không vượt quá tồn kho)
        if ($newQuantity > $product->quantity) {
            return response()->json(['success' => false, 'message' => 'Số lượng vượt quá mức tồn kho.']);
        }
